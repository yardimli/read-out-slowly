<?php
	...
	use Exception; // Keep standard Exception
	...
	class SimplifiedLlmAudioHelper
	{
		...
		public static function init(array $config = []): void
		{
			...
			if (!is_dir($logDirectory)) {
				if (!mkdir($logDirectory, 0775, true) && !is_dir($logDirectory)) {
					...
					// Optionally, you could throw an exception here if logging is critical
				}
			}
			...
			if (self::$loggingEnabled && is_writable($logDirectory)) {
				...
				if ($logDirectory) { // Only log error if a directory was attempted
			...
			self::log('DEBUG', 'Config loaded: ' . json_encode($config)); // Log the actual config
		}
		...
		private static function log(string $level, string $message, $context = null): void
		{
			if (!self::$loggingEnabled || empty(self::$logFilePath)) {
				// If logging is disabled or no path, optionally log to error_log as a fallback for critical messages
			...
			// Use FILE_APPEND to add to the file, LOCK_EX for safety (though less critical for simple append)
		...
		public static function sendTextToLlm(
			...
			self::log('DEBUG', "SimplifiedLlmAudioHelper::sendTextToLlm Request Data: ", $data); // Pass array as context
			...
			while ($attempt <= $max_retries && $llm_response_content === null) {
				...
				try {
					...
					self::log('ERROR', $last_error_message . ' Trace: ' . $e->getTraceAsString());
					if ($attempt > $max_retries) break;
					sleep(pow(2, $attempt));
				}
			}
		...
		public static function textToSpeechOpenAI(
			...
			$baseFilename = preg_replace('/[^a-z0-9_.-]+/', '-', strtolower($outputFilenameBase . '_' . $uniquePart));

			$rawOutputFilename = $baseFilename . '_raw.mp3';
			$amplifiedOutputFilename = $baseFilename . '_amplified.mp3';
			...
			self::log('INFO', "SimplifiedLlmAudioHelper::textToSpeechOpenAI called.", ['voice' => $voiceName, 'model' => $openAiModel, 'text' => substr($text, 0, 70) . "..."]);
